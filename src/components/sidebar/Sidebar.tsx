  ...
  return (
    <>
      <nav
        ...
        style={{ ...styleSidebar, position: isMobile ? 'absolute' : 'relative' }}
      >
        <Link to='/' className='flex gap-2 items-center'>
          <MainIcon size={isSidebarVisible !== null ? 'xl' : 'md'} />
          ...
          </ul>
          {
            isMobile && (
              <Button onClick={handleSidebarVisibility} className='px-1 mt-5'>
                {isSidebarVisible === true ? <AiOutlineLeft /> : <AiOutlineRight />}
              </Button>
            )
          }
        </div>
      </nav>
      {isMobile && isSidebarVisible === false && (
        <Button
